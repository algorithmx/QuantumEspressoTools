...
settings1 = copy(band_w90_additional_settings_default)
settings1["diag"] = ["david","cg"]
settings1["beta"] = [0.8, 0.5, 0.3]
...
if iX > length(NX_LIST)
    ...
    band_w90_kgrid(
        ...
        "SSSP_Efficiency",               #*  ps_mode
        (1,12,1,0,0,0),                  #*  kpoint_config::NTuple{6,Int64},
        ("crystal_b", 
         [(0.0, 0.0, 0.0) => 40, 
          (0.0, 1.0, 0.0) => 1,]),
        (45.0,350.0),                    #*  cutoff::Tuple{Float64,Float64},
        32*Nux,                          #*  nbnd
        (10*Nux,15*Nux),
        ["C:pz",];
        additional_settings = settings1,
        cleanup     = false,
