...
function parse_electron_config(c::String)
    s = [c[l] for l ∈ findall(r"\ds\d",c)]
    p = [c[l] for l ∈ findall(r"\dp\d",c)]
    d = [c[l] for l ∈ findall(r"\dd\d0*",c)]
    f = [(last(l)==length(c) ? c[l] : (isdigit(c[last(l)+2]) ? c[first(l):last(l)+1] : c[l])) for l ∈ findall(r"\df\d",c)]
    return s,p,d,f
end
